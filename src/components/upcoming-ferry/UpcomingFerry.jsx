const UpcomingFerry = () =>{
    return(
        <div>
            {/* upper tabs */}
            <div className="max-w-[950px] m-auto grid grid-cols-4 shadow-button-shadow overflow-hidden sm:rounded-tl-[25px] rounded-tl-[15px] sm:rounded-tr-[25px] rounded-tr-[15px]">
                {upcomingFerryTabs?.map((item,index)=>{
                    return(
                        <div key={index} className={`sm:p-6 p-[4px] sm:text-[15px] text-[8px] text-center border-r ${index===0 ? "bg-white text-[#0A1951]" : "text-white bg-[#8890B1]"}`}>{item?.tab}</div>
                    )
                })}
            </div>
            {/* lower section */}
            <div className="max-w-[1150px] m-auto shadow-button-shadow sm:rounded-[30px] sm:p-8 p-2 sm:rounded-bl-[25px] rounded-bl-[15px] sm:rounded-br-[25px] rounded-br-[15px]">
                        ...
                        <div key={index} className="grid sm:grid-cols-[90%_10%] grid-cols-[80%_20%] gap-4 w-full items-center border-b-2 border-[#dde0ee] mb-4">
                            <div className="flex justify-between items-center border-r-2 border-[#dde0ee] pr-4 mb-4">
                                ...
                                <div className="text-center sm:px-6 px-2">
                                    ...
                                    <p className="text-[#0A1951] sm:text-[15px] text-[10px]">Nautika Lite</p>
                                </div>
                                <div className="sm:block hidden">
                                <img src="/svg/dashed.svg" alt="line" className="w-full" />
                                </div>
                                <div className="text-center sm:pl-6 sm:text-[15px] text-[10px]">
